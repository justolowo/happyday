...
You can download the original R notebook document [HERE](http://spatial.binghamton.edu/geog533/labs/Geog533_lab02_solutions.nb.html)
...
### Question 1a
find out the cheapest car in each type
```{r}
library(MASS)
df <- Cars93
...
### Question 1b
find out the greatest fuel efficiency in each type
```{r}
...
The skewness is **`r skewness(df$power.diff)`**; the kurtosis is **`r kurtosis(df$power.diff)`**
...
library(DT)
datatable(df.USA,options = list(pageLength = 5),caption = "USA")
datatable(df.nonUSA,options = list(pageLength = 5),caption = "non-USA")
```
...
write.csv(df.USA,"output/carsUSA.csv")    ### write to a csv file
USA.csv <- read.csv("output/carsUSA.csv")   ### read the csv file
str(USA.csv)
library(DT)
datatable(USA.csv,options = list(pageLength = 5),caption = "USA")
```
